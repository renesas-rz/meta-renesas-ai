...
parser = argparse.ArgumentParser(description='Inception v3 Inference')
...
inception_v3.load_state_dict(torch.load(model_location))

with open(label_location) as f:
    labels = [line.strip() for line in f.readlines()]

transform = transforms.Compose([
    transforms.Resize(299),                    #resize image to 256x256
    transforms.CenterCrop(299),                #crop image to 224x224
...
out = inception_v3(batch_transform) # Warm up

for i in range(inference_count):
    start_time = time.time()
    out = inception_v3(batch_transform)
